...
for counter in range(1_000):
	flux_planet, flux_moon, testdata, px_bary, py_bary, mx_bary, my_bary = pandora_model(
	    r_moon,
	    a_moon,
	    tau_moon,
	    Omega_moon,
...
plt.plot(time_array, testdata, color="green", linestyle="dashed")
#plt.show()
...
flux_total_array = pandora_vectorized(
	    ...
	    np.full(10, a_moon),
	    np.full(10, tau_moon),
	    np.full(10, Omega_moon),
	    np.full(10, w_moon),
	    np.full(10, i_moon),
	    np.full(10, M_moon),
	    np.full(10, per_planet),
	    np.full(10, a_planet),
	    np.full(10, r_planet),
	    np.full(10, b_planet),
	    np.full(10, t0_planet),
	    np.full(10, M_planet),
	    R_star,
	    u,
	    time_array
		)
#print(flux_total_array[4])
...
for idx in range(len(flux_total_array)):
	#print(flux_total_array[idx])
	loglike = -0.5 * ((flux_total_array[idx] - testdata) / yerr**2).sum()
	loglikes[idx] = loglike
print(loglikes)


#loglikes = -0.5 * (((flux_total_array - testdata) / yerr)**2).sum()
#print(loglikes)
