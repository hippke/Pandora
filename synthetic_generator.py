...
noise = np.random.normal(0, noise_level, len(time_arrays))
testdata = noise + flux_total
yerr = np.full(len(testdata), noise_level)

print(np.array((time_arrays, testdata)))

np.savetxt("output.csv", np.transpose(np.array((time_arrays, testdata))), fmt='%8f')
